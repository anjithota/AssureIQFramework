...
public class CCDN_ForceClosure_Drop_TC extends ActionEngine {
	String Epath = "./TestData/ChangeControlSR.xlsx";

	public CCDN_ForceClosure_Drop_TC() {
	...
	@Test(priority=9, dataProvider="DnFCDrop", enabled = true)
	public void docForceClosurerop(String RemarkReason) {
		this.test = extent.createTest("Documentatoin Action Plan Force Closure Drop");
		epiqLogin.loginToEPICOQApplication(ConfigsReader.getPropValue("CCLgnIHODID"),
				ConfigsReader.getPropValue("CCLgnIHODPwd"));
		ccdnForceClosureApproval.ccForceClosureDrop(RemarkReason);
		ccdnForceClosureApproval.forceClosureDroppedAuditTrail();
		logout.logOut();
	}
